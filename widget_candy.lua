...
V._CheckProps = function ( Widget)
	...
	if V.Themes [Props.theme] == nil then V.error("!!! WIDGET ERROR: _CheckProps(): Unknown theme '"..Props.theme.."' for widget '"..Props.name.."'."); return end
...
V.Show = function (name, state, animTime) 
	...
	if animTime ~= nil then
		...
		Widget.FadeTrans = transition.to(Widget, {time = animTime, alpha = alpha, onComplete = function() Widget.alpha = Widget.Props.alpha; Widget.isVisible = state end})
	else
...
V.NewShapeButton = function (Props)
	...
	function Grp:_update()
		...
		if #Props.border > 0 then
			...
			Tmp.strokeWidth  = 0
			Tmp:setFillColor (255,255,255,32)
			self[2]:insert(Tmp)
		end
		...
		if Props.caption ~= nil and Props.caption ~= "" then
			...
			self[4].xScale = Theme.ButtonIconSize / self[4].width
			self[4].yScale = Theme.ButtonIconSize / self[4].height
...
V.NewDragButton = function (Props)
	...
	function Grp:_create()
		...
		Tmp:setReferencePoint(display.CenterRightReferencePoint)
		Tmp.y            = 0
		Bubble:insert(Tmp)
		-- BUBBLE M
...
V.NewList = function (Props)
	...
	function Grp:_update(animate)
		...
		-- SCROLLBAR
		while self[15].numChildren > 0 do self[15][1]:removeSelf() end
		if Props.scrollbar ~= "never" then
			local barH       = (self.numItems / #Props.list); if barH > 1 then barH = 1 elseif barH <= 0 then barHeight = 0.001 end
			local barAlpha   = Props.scrollbarAlpha ~= nil and Props.scrollbarAlpha or 255

			-- SCROLLBAR BG
			Tmp = display.newRoundedRect(w-12,3,8,h-6,3)
			Tmp.strokeWidth = 1
			Tmp:setFillColor  (255,255,255, barAlpha)
			Tmp:setStrokeColor(0,0,0, 200)
			self[15]:insert(Tmp)

			-- SCROLLBAR THUMB
			Tmp = display.newRoundedRect(w-11,4,6,(h*barH)-6,3) 
			Tmp.strokeWidth = 0
			Tmp:setFillColor     (0,0,0, 200)
			Tmp:setStrokeColor   (255,255,255, 0)

			Tmp:setReferencePoint(display.TopCenterReferencePoint)
			Tmp.maxY  = h-Tmp.height
			self[15]:insert(Tmp)
			self:_positionScrollbar(true)
			if Props.scrollbar == "onScroll" then self[15].alpha = 0 end
		end
	end
	...
	function Grp:_positionScrollbar(hide)
		...
		if self.listMinY >= 0 or self.Props.scrollbar == nil or self.Props.scrollbar == "never" then
			self[15].alpha = 0; return
		else 	self[15].alpha = 1 end
		-- POSITION SCROLLBAR
		self[15][2].y  = (self.listY / (self.listMinY/100) / 100) * (self[15][2].maxY-6) + 3 
	end
		...
		if self.lastOffset ~= self.itemOffset then
			...
			for i = 1,Items.numChildren do
				...
				if num == Props.selectedIndex then
					 ...
					 Item[2]:setFillColor(Theme.ListItemSelectedGradient)
				...
				if isTable then
					...
					if Props.disableBGColor ~= true and num ~= Props.selectedIndex then
						...
						if data.bgColor ~= nil then 
							if data.bgColor[4] == nil then data.bgColor[4] = 255 end
							Item[2]:setFillColor(data.bgColor[1],data.bgColor[2],data.bgColor[3],data.bgColor[4]) 
						else
							Item[2]:setFillColor(Theme.color[1],Theme.color[2],Theme.color[3],64) 
						end
					end
					-- RIGHT ICON
					...
					if data.image ~= Item.imgFile or data.imageSize ~= Item.imgSize then
						...
						Item.imgSize = data.imageSize ~= nil and data.imageSize or Theme.ListIconSize
				...
				if Item[4].isVisible then     -- HAS RIGHT ICON?
					maxTextW = maxTextW - (Theme.ListIconSize + 16) 
				end
				...
				-- SET COLOR
				if isTable == false or data == nil then
					Item[3]:setTextColor(textColor[1],textColor[2],textColor[3],255)
					Item[4].isVisible = false
					Item[5].isVisible = false
					if Item[6] then Item[6]:removeSelf(); Item.imgFile = nil end
				else
					if data.textColor then Item[3]:setTextColor(data.textColor[1],data.textColor[2],data.textColor[3],255) 
					                  else Item[3]:setTextColor(textColor[1],textColor[2],textColor[3],255) end
				end
				-- POSITION TEXT
				...
				elseif Item[5].isVisible == true then -- HAS LEFT ICON?
					Item[3].x = Item[3].x + Theme.ListIconSize + 4 
				end
				Item[3].y = itemH * .5
			end
		end
	...
	function Grp:_drawPressed(event) 
		...
		elseif Props.editMode == true and event.ly > Props.Shape.h - size then
			...
			return
		-- DEL ICON PRESSED?
		elseif Props.editMode and Props.allowDelete and ((List[item].imgFile == nil and event.lx < 8 + Theme.ListIconSize) or (List[item].imgFile ~= nil and event.lx > (Props.Shape.w - 8 - Theme.ListIconSize) ) ) then
			...
			for i = 1,Items.numChildren do
				...
				if isTable then
					...
					if Props.disableBGColor ~= true and num ~= Props.selectedIndex then
						Item[2].isVisible = true
						if data.bgColor ~= nil then 
							Item[2]:setFillColor(data.bgColor[1],data.bgColor[2],data.bgColor[3],64) 
						else
							Item[2]:setFillColor(Theme.color[1],Theme.color[2],Theme.color[3],64) 
						end
					end
				...
				if data and data.content then
					...
					Item[3][1]:setReferencePoint(display.TopLeftReferencePoint)
					Item[3][1].x = 0
					Item[3][1].y = 0
...
V.NewIconBar = function (Props)
	...
	function Grp:_update()
		...
		if Props.lastIcons ~= Props.icons then
			...
			while self[3].numChildren > 0 do self[3][1]:removeSelf() end
		...
		local tmp     = (#Props.icons * size) + ((#Props.icons-1) * spacing)
		local pos     = 0
		local Icon
		for i = 1, #Props.icons do
			Icon = self[3][i]
			...
			if Props.icons[i].disabled == true then
			     Icon.alpha = 0.5; Icon:setFillColor(100,100,100)
			else Icon.alpha = 1.0; Icon:setFillColor(255,255,255) end
			-- HORIZONTAL / VERTICAL?
			if w > h then Icon.x = pos; Icon.y = 0
				 else Icon.x = 0  ; Icon.y = pos end
			pos = pos + spacing
		end
		...
		-- MARKER
		if Props.marker == nil then Props.marker = {8,1, 255,0,0,50,  185,185,185,150} end
		self[2].x = self[3].x
		self[2].y = self[3].y
		...
		-- TOP SHINE
		if self[4][1] then self[4][1]:removeSelf() end
		if #Props.border > 0 then
			if w > h then Tmp = display.newRoundedRect(4,4,w-8,16,4)
					 else Tmp = display.newRoundedRect(4,4,16,h-8,4) end
			Tmp.strokeWidth  = 0
			Tmp:setFillColor (255,255,255,Props.glossy ~= nil and Props.glossy or 32)
			self[4]:insert(Tmp)
		end
	...
	function Grp:setMarker(pos)
		...
		self[2][1].x      = self[3][pos].x
		self[2][1].y      = self[3][pos].y
	...
	function Grp:updateIcon(pos, Values)
		...
		local Icon = self[3][pos]
		if Props.icons[pos].image ~= nil then
			local x,y = Icon.x,Icon.y
			Icon:removeSelf()
			Icon = display.newImageRect(Props.icons[pos].image, Props.icons[pos].baseDir, Props.iconSize, Props.iconSize) 
			Icon.x,Icon.y,Icon.scale = x,y,1.0
			self[3]:insert(pos, Icon)
		elseif Props.icons[pos].icon > 0 then
			 Icon:setFrame  ( Props.icons[pos].icon )
			 Icon.isVisible = true
		else Icon.isVisible = false end
		-- DISABLED?
		if Props.icons[pos].disabled == true then
			 Icon.alpha = 0.5; Icon:setFillColor(100,100,100)
		else Icon.alpha = 1.0; Icon:setFillColor(255,255,255) end
	end
...
V.Confirm = function (Props)
	...
	for i = 1, #Props.buttons do
		...
		Tmp = V.NewButton(
			...
			onRelease       = function(event) event.button = event.Props.myNumber; releaseFunc(event); V.Widgets[event.Props.myWindow]:destroy() end, 
			} )
	end
	Win:layout(true)
